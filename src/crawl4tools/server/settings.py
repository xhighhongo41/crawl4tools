...
class ServerSettings:
    ...
    def __post_init__(self) -> None:
        ...
        Raises:
            ValueError: if concurrency/max_urls/timeout_s are out of range,
                or the proxy URL is invalid.
        """
        if self.concurrency < 1:
            raise ValueError(f"concurrency must be at least 1, got {self.concurrency}")
        if self.max_urls < 1:
            raise ValueError(f"max_urls must be at least 1, got {self.max_urls}")
        if self.timeout_s <= 0:
            raise ValueError(f"timeout_s must be greater than 0, got {self.timeout_s}")
    ...
    def fetch_options(
        ...
        *,
        ...
        Raises:
            ValueError: if *timeout_s* is given and not greater than 0.
        """
